...
def run_tpcds(con):
    for i in range(1, 100):
        print(con.sql(f"PRAGMA tpcds({i})"))
...
with duckdb.connect() as con:
    ...
    # Measure the time taken to initialize the TPC-H database
    measure_time(lambda: con.sql("CALL dsdgen(sf = 10)"))

    # Measure the time taken to run the TPC-H queries
    measure_time(lambda: run_tpcds(con))
